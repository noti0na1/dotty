...
private sealed trait XSettings:
  ...
  val XreplEvalLogDir: Setting[String] = StringSetting(
    ...
    "Xrepl-eval-log-dir",
    "directory",
    "Directory to write per-invocation logs of dynamic `eval(...)` calls. " +
      "For each call, three files are written: " +
      "`eval_<timestamp>_enclosingSource.scala` (the enclosing top-level statement with a placeholder marking the eval call site), " +
      "`eval_<timestamp>_code.scala` (the body the user submitted), and " +
  ...
  val XreplHistoryFile: Setting[String] = StringSetting(
    ...
    "Xrepl-history-file",
    "file",
    "Path of a file to which the REPL appends a transcript of each line. " +
      "Each entry is the user's input followed by everything written to the " +
      "REPL's output (rendering of results, user `println`s, stack traces) " +
      "while that line was being interpreted. ANSI color codes are stripped. ",
    ""
  )
